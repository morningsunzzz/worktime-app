...
async def clock_in():
    ...
    row = await pool.fetchrow("SELECT * FROM work_records WHERE id = $1", record_id)
...
async def add_record(r: WorkRecordIn):
    pool = await get_pool()
    settings = await crud.get_settings(pool)
    clock_in_dt = r.clock_in if isinstance(r.clock_in, datetime) else datetime.fromisoformat(str(r.clock_in))
    if r.clock_out:
        clock_out_dt = r.clock_out if isinstance(r.clock_out, datetime) else datetime.fromisoformat(str(r.clock_out))
...
async def edit_record(id: str, r: WorkRecordIn):
    ...
    row = await pool.fetchrow("SELECT * FROM work_records WHERE id = $1", id)
    return _record_from_row(row)
